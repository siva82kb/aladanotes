...
$\mf{x}^\top\mf{y}$ is the standard inner product, which of course means there are non-standard inner products. The concept of an inner product is a general one - it is a function that maps two vectors to a scalar value, and satisfies the following properties:
\begin{itemize}
    \item \textbf{Symmetry:} For any vectors $\mf{x}, \mf{y} \in \mb{R}^n$, $\mf{x}^\top\mf{y} = \mf{y}^\top\mf{x}$.
    \item \textbf{Linearity:} For any vectors $\mf{x}, \mf{y}, \mf{z} \in \mb{R}^n$ and any scalars $a, b \in \mb{R}$, $\lp a\mf{x} + b\mf{y} \rp^\top\mf{z} = a\mf{x}^\top\mf{z} + b\mf{y}^\top\mf{z}$.
    \item \textbf{Positive definiteness:} For any vector $\mf{x} \in \mb{R}^n$, $\mf{x}^\top\mf{x} \geq 0$, and $\mf{x}^\top\mf{x} = 0$ if and only if $\mf{x} = \mf{0}$.
\end{itemize}


The standard inner product is also called the \textit{dot product} or the \textit{scalar product}. The standard inner product is a special case of a more general class of inner products called the \textit{inner product}. The inner product of two vectors $\mf{x}, \mf{y} \in \mb{R}^n$ is a function that maps two vectors to a scalar value, and satisfies the following properties:
