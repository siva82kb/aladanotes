...
\end{itemize}
We will come across other inner products in due course, but we will stick to the standard inner product for most problem in $\mb{R}^n$ in this course.
...
\begin{problem}
    ...
    %     \item $\langle \mf{x}, \mf{y}\rangle = \sum_{i=1}^n w_i x_i y_i$, $\,\, w_i = \begin{cases} +1, & i \text{ is odd} \\  -1, & i \text{ is even}\end{cases}$.
    % \end{enumerate}
\end{problem}
\end{boxedstuff}
