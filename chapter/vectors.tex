...
%       \captionof{figure}{An example figure inside a tcolorbox}
%     \end{center}
...
%         \hline
%       \end{tabular}
%       \captionof{table}{An example table inside a tcolorbox}
%     \end{center}
%   \end{tcolorbox}
...
\begin{boxedstuff}
    \begin{example}
        ...
        % \begin{table}[]
            ...
            % 1 & Sex & \begin{tabular}[c]{@{}c@{}}None\\ (0 - Male, 1- Female)\end{tabular} \\
            % 2 & Age & Years \\
            % 3 & Height & cm \\
            % 4 & Weight & kg \\
            % 5 & Heart rate & count \\
        ...
        Sex, Age, heart rate, systolic pressure, diastolic presure, and temperature; we assume that categorical variables are given numerical lables. The following are some examples of $n$-vectors generated from three different patients visiting the hospital.
        % \begin{equation*}
        %     \mf{x}_1 = \bmxc 1 \\ \emx \quad \mf{x}_2 = \bmxc \emx \quad \mf{x}_3 = \bmxc \emx
        % \end{equation*}
    \end{example}
\end{boxedstuff}
...
Linear independece is a \textit{property} of a set of vectors; a set of vector is either linearly independent or linearly dependent. The concept of linear independence is easy to understand the but the algebraic condition for independence can seem a bit unintuitive. A set of vectors is said to be \text{linearly independent} if no vector in the set can be expressed as a linear combination of the other vectors in the set. This means that there is some unique  information contained in each element of the set, which cannot be obtained from the other elements of the set, i.e., there is no redundancy, so to speak. 
...
\secsum{
    \begin{itemize}
        ...
        \item The span of a set of vector $V$ is a subsapce of the originak vectors space the elements of $V$ are from.
    \end{itemize}
}
...
\begin{enumerate}
    ...
    \item \textcolor{blue}{\textbf{[Programming]}} Let's build a simple classifier using the concepts you've learned in this chapter. We will pretend that we are doing this for classifying or detecting the presence or absence of a disease called - \textit{vector space sickness}. We wish to do this through two clinical tests - (1) \textit{Subspace assay} and (2) \textit{Basis balance scale}. Both these serious clinical tests generate numerical outcomes that can assume any real number value.
    
    The department has been conducting a large scale clinical study for the last 5-6 years collecting data from participants, from different background, with and without vector space sickness by administering the subspace assay and the basis balance test. The data from this study is stored as a CSV file with four columns: (a) \textcolor{blue}{\texttt{subjectno}} -- subject numbers, (b) \textcolor{blue}{\texttt{x1}} --  value of the subspace assay test, (c) \textcolor{blue}{\texttt{x2}} -- value of the basis balance scale, and (d) \textcolor{blue}{\texttt{vss}} -- presence (1) or absence (0) of the vector space sickness condition. Each row of this CSV file corresponds to a individual subject who participated in the study. Your goal here is to look at the data from this experiment and propose how we could build a classifier for determine if a person has vector space sickness is we are given their scores on the subspace assay and the basis balance scale clinical tests. 

    \begin{enumerate}
        \item \textbf{Dataset from young MTechs}. One group of participants in the study was a a large sample of  of young, 1st year MTech students. Unfortunately, almost half of these students were diagnosed with vector space sickness. This data is stored in \textcolor{blue}{\texttt{expt1.csv}}. Read this data, make a scatter plot in 2D (\texttt{x1} versus \texttt{x2}) with the data points for participants with vector space sickness in blue and the one without vector space sickness in red. Look at the data, and propose how you could use the measurements \texttt{x1} and \texttt{x2} to distinguish between participants from the two groups. Implement the classifier you've proposed and find out how well it performs in correctly classifying the two groups.
        
        \item \textbf{Messed up dataset}. When collecting data from a second set of participants (similar to the one in \textcolor{blue}{\texttt{expt1.csv}}), the engineer had written a program program and ended up saving the results from the subspace assay in a different units. This data is available in \textcolor{blue}{\texttt{expt2.csv}}. Surely, the classifier you've developed using expt1.csv should work here as well, as the data is froma similar population, we are using the same two clinical tests, except for the change of units in one of the tests. Run the classifier you've developed using \textcolor{blue}{\texttt{expt1.csv}} on data from \textcolor{blue}{\texttt{expt2.csv}}. Does the classifier work as well? If yes, great! You can move to the next question. If not, why? How can we fix this problem?
    \end{enumerate}
\end{enumerate}
